...
if ( ! function_exists( 'qc_clr_comments_include_promo_page_scripts' ) ) {	
	function qc_clr_comments_include_promo_page_scripts( ) {   
        ...
        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "qcclr_comment_supports"  ) ){

            wp_enqueue_style( 'qcld-support-fontawesome-css', qc_clr_comments_support_url . "css/font-awesome.min.css");                              
            wp_enqueue_style( 'qcld-support-style-css', qc_clr_comments_support_url . "css/style.css");

            wp_enqueue_script( 'jquery' );
            wp_enqueue_script( 'jquery-ui-core');
            wp_enqueue_script( 'jquery-ui-tabs' );
            wp_enqueue_script( 'jquery-custom-form-processor', qc_clr_comments_support_url . 'js/support-form-script.js',  array('jquery', 'jquery-ui-core','jquery-ui-tabs') );

            wp_add_inline_script( 'jquery-custom-form-processor', 
                                    'var qc_clr_comments_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qc_clr_comments_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
