...
add_action('admin_head', 'qc_clr_free_ai_admin_submenu_css');
if ( ! function_exists( 'qc_clr_free_ai_admin_submenu_css' ) ) {
    function qc_clr_free_ai_admin_submenu_css() {
        echo '<style>
            #adminmenu .wp-submenu li a[href*="admin.php?page=qc_clr_free_ai"] {
...
if ( ! function_exists( 'qc_clr_free_ai_include_promo_page_scripts' ) ) {	
	function qc_clr_free_ai_include_promo_page_scripts( ) {   
        ...
        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "qc_clr_free_ai"  ) ){
                             
            wp_enqueue_style( 'qc_clr_free_ai_css', qc_clr_free_ai_support_url . "css/qc-ai-free-style.css");

            wp_enqueue_script( 'jquery' );

            wp_enqueue_script( 'qc_clr_free_ai_js', qc_clr_free_ai_support_url . 'js/qc-ai-free-script.js',  array('jquery') );

            wp_add_inline_script( 'qc_clr_free_ai_js', 
                                    'var qc_clr_free_ai_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qc_clr_free_ai_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
            ...
            <div class="qc_clr_free_ai-wrapper">
                ...
                <div class="qc_clr_free_ai-footer">
                    ...
                    <div class="qc_clr_free_ai_btn-group">
                        <a href="<?php echo esc_url('https://www.wpbot.pro/docs/'); ?>" target="_blank" class="qc_clr_free_ai_btn-docs"><?php esc_html_e('Knowledge Base', 'qc-opd'); ?></a>
                        <a href="<?php echo esc_url('https://www.wpbot.pro/free-support/'); ?>" target="_blank" class="qc_clr_free_ai_btn-support"><?php esc_html_e('Get Direct Support', 'qc-opd'); ?></a>
                    </div>
